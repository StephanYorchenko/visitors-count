...
class SQLExecutorTests(unittest.TestCase):
	...
	def test_initialize(self):
		...
		self.assertListEqual(a, [('page_id', 'INTEGER'),
								 ('client_id', "TEXT"),
								 ('visit_time', 'TEXT')])

	def test_execute(self):
		sqls = ['SELECT * FROM visits;',
	...
	def test_insert(self):
		self.sql_executor.try_insert('visits',
									 page_id=13,
	...
	def test_select_by_dates(self):
		...
		records = [(156, "qwertyuio", last_time),
		...
		self.assertEqual(len(self.sql_executor.select_for_days("-1 day", 156)),
						 2)
	...
	def test_count_all(self):
		...
		self.assertEqual(
				self.sql_executor.count_all(156, '-1 day'), 3)
		self.assertEqual(
